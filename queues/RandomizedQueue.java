...
/**
 * The {@code RandomizedQueue} class represents a 
 * randomized queue of generic items. It is similar to stack or 
 * queue, except that the item removed is chosen uniformly 
 * at random among the items in the queue. It supports the usual
 * enqueue and dequeue along with other methods of sampling 
 * sampling a random item, testing if the queue is empty, getting 
 * the number of items in the queue, and iterating over the items 
 * in uniformly random order.
 *
 * @param <Item> the generic type of each item in the queue
 * @author Karthikeyan
 */
public class RandomizedQueue<Item> implements
Iterable<Item> {
	...
	/** 
	 * Returns an iterator that iterates over the item in random 
	 * order 
	 *
	 * @return an iterator that iterates over the item in random 
	 * order
	 */
