...
/**
  * Percolation, given a composite systems comprised of        
  * randomly distributed insulating and metallic materials:
  * what fraction of the materials to be metallic so that the
  * composite system is an electrical conductor? Given a
  * porus landscape with water on the surface (or oil below),
  * under what conditions will the water be able to drain 
  * through to the bottom (or the oil to gush through to the
  * surface)? Scientists have defined an abstract process
...
public class Percolation {
	...
	/** 
	  * Creates an n-by-n grid, with all the sites are blocked and
	  * initialises all the object and connects the virtual top to 
	  * the top n sites and the virtual bottom to the bottom n
	  * sites.
	  *
	...
	public Percolation(int n) {
		...
		virtualTop = 0; 
		virtualBtm = n * n + 1;
		//connects the top and bottom sites to the virtual top 
		// and the virtual bottom.
		for (int i = 1; i <= n; i++) {
	...
	/**
	  * Opens a site if it not open and connect with adjacent 
	  * opened sites.
	  *
	  * @param row  row index of the site (1-based)
	  * @param col column index of the site (1-based)
	  * @throws IllegalArgumentException if row or col are 
	  * out of bounds 
	  */	
	...
	/**
	  ...
	  * @throws IllegalArgumentException if row or col are
	  * out of bounds 
	  * @return {@code true} if the site is open; {@code false}
	  * otherwise 
	  */		
	...
	/**
	  * Returns true if the given site is opened and connected to 
	  * the top. 
	  *
	  * @param row row index of the site (1-based) 
	  * @param col column index of the site (1-based)
	  * @throws IllegalArgumentException if row or col are
	  * out of bounds 
	  * @return {@code true} if the site is opened and connected 
	  * to the top; {@code false} otherwise
	  */	
	...
	/**
	  ...
	  *
	  * @return {@code -1} if row or col are out of bounds; 
	  * index otherwise
	  */
    ...
    {	return isInvalid(row, col) ? -1 : (row - 1) * n + col; }
     ...
     /**
       ...
       * @param row row index of the site (1-based)
       * @param col column index of the site (1-based) 
       * @return {@code true} if row or col is out of bounds;
       * {@code false} otherwise 
       */
